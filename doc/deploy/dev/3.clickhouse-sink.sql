...
(
    ...
    `team_id` String COMMENT '企业ID',
    `guid` String COMMENT '文件ID',
    `name` String COMMENT '文件名称',
    `file_views` Int64 COMMENT '文件访问量',
    `row_number` Int64 COMMENT '名次'
) ENGINE = MergeTree()
    PARTITION BY toYYYYMM(ldate)
    ORDER BY ldate;
...
from all.dws_collaboration_7d_statistic_by_member_daily where is_seat=1;
...
create view all.dws_enterprise_30d_active_user_statistic_by_global_daily_view on cluster "shard2-repl1" as select
    ...
    user_name,
    (file_views * 1 + create_files * 10 + (add_collaborations + public_shares + comments + use_ats) * 5)  as active_score
from all.dws_collaboration_30d_statistic_by_member_daily where is_seat=1;
