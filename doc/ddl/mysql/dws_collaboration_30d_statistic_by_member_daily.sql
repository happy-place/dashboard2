...
delete from boss.dws_collaboration_30d_statistic_by_member_daily where ldate='2020-11-26';


-- 3) 尝试查询
select * from boss.dws_collaboration_30d_statistic_by_member_daily where ldate = '2020-11-26';


-- 4）创建视图计算 最近30天最活跃成员 活跃分= 浏览数*1 + 生产数（即新建文件数）*10 + 协作行为（包括添加协作、公开分享、评论次数、@人次数）*5
...
select * from boss.dws_collaboration_30d_statistic_by_member_daily;

-- 5） 查询 一周最活跃成员
select * from boss.dws_enterprise_30d_active_user_statistic_by_global_daily_view where ldate = '2020-11-26';