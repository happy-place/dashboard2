...
-- 2) 新建表
alter table dws_file_1d_hot_statistic_by_global_daily modify column  `name` longtext COMMENT '文件名称';
CREATE TABLE boss.dws_file_1d_hot_statistic_by_global_daily
(
    `ldate` Date COMMENT '日期：最近1天最后一天',
...
-- 3) 尝试查询
select * from boss.dws_file_1d_hot_statistic_by_global_daily where ldate = '2020-11-26';